import { FC, ReactNode, useState, useEffect, useMemo, useCallback } from 'react';
...
import { fetchGithubUser, fetchAggregateGithubUserData, fetchGithubUserStatisticsTrend } from '@/apis/github';
...
export const CowboyDashboard: FC = () => {
  ...
  const [aggregateGithubUserData, setAggregateGithubUserData] = useState({ user: {}, rateLimit: {} })
  ...
  const commitTotal = useMemo(() => {
    // 先检查必要的属性是否存在
    if (!githubUserStatisticsTrend?.commitsTrend?.commitContributionsByRepository) {
      return 0; // 或者返回其他默认值
    }
    return githubUserStatisticsTrend.commitsTrend?.commitContributionsByRepository.reduce((acc, pre) => {
  ...
  const commitAndPrList = useMemo(() => {
    ...
    const formatPullRequestsTrend = prList.map(
      (item: IPullRequestNode) => ({
    ...
    return [...formatCommitTrend, ...formatPullRequestsTrend]
      ...
      .slice(0, 35)
  }, [aggregateGithubUserData.user?.repositories?.nodes, githubUserStatisticsTrend?.pullRequestsTrend?.nodes])
      ...
      <div className="lg:col-span-3 flex flex-col gap-6">
        ...
        <div className="bg-[#fdfbf7] p-4 rounded shadow-xl border-2 border-[#d7c4a1] relative overflow-hidden group">
          ...
          <div className="grid grid-cols-2 gap-2 mt-6">
            <StatBox label="Commit" value={commitTotal} icon={<GitCommit size={14}/>} />
            <StatBox label="Star" value={aggregateGithubUserData.user.starredRepositories?.totalCount ||0} icon={<Star size={14}/>} />
            <StatBox label="PR" value={githubUserStatisticsTrend.pullRequestsTrend?.totalCount - 3 ||0} icon={<GitPullRequest size={14}/>} />
            <StatBox label="Issue" value={githubUserStatisticsTrend.issuesTrend?.totalCount ||0} icon={<CircleDot size={14}/>} />
          </div>
        </div>
      ...
      <div className="lg:col-span-5 flex flex-col gap-6">
        ...
        <div className="bg-[#fdfbf7] p-4 rounded shadow-xl border border-[#d7c4a1]">
          ...
          <div className="flex flex-col gap-4">
            {[0, 1].map(i => (
              <GithubCalendar key={i} year={year - i} weeks={aggregateGithubUserData.user[`Y${year - i}`]?.contributionCalendar.weeks} />
            ))}
        ...
        <div className="bg-[#fdfbf7] p-4 rounded shadow-xl border border-[#d7c4a1] flex-grow">
          ...
          <GithubRepo list={aggregateGithubUserData.user?.repositories?.nodes || []}></GithubRepo>
        </div>
      </div>
      ...
      <div className="lg:col-span-4 flex flex-col gap-6">
        <div className="bg-[#fdfbf7] p-4 rounded shadow-xl border border-[#d7c4a1] h-full relative">
           ...
           <div className="absolute left-2 top-0 bottom-0 flex flex-col justify-around py-4">
              {[...Array(10)].map((_, i) => <div key={i} className="w-3 h-3 rounded-full bg-stone-200 shadow-inner"></div>)}
           </div>
...
const StatBox = ({ label, value, icon }: { label: string, value: string | number, icon: ReactNode }) => (
